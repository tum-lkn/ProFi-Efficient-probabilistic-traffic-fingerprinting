...
def calculate_quality_measurement(y_true: List[int], y_pred: List[int]) -> Tuple[float, float, float, float, np.array]:
...
def check_assignment(browser) -> Dict[str, Any]:
    ...
    if confusion_matrix.shape != (len(applications) + 1, len(applications) + 1):
        confusion_matrix = np.vstack((confusion_matrix, np.zeros((1, len(applications)))))
        confusion_matrix = np.hstack((confusion_matrix, np.zeros((len(applications) + 1, 1))))
